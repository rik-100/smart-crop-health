...
function handleFiles(fileList) {
  ...
  const newFiles = Array.from(fileList).filter(f => allowed.includes(f.type) && f.size <= 20*1024*1024);
  
  if (newFiles.length === 0) {
    showToast(t("toast_invalid_file") + " or file too big.");
    return;
  }
  currentFiles = currentFiles.concat(newFiles);
  renderThumbnailTray();
  selectThumbnail(currentFiles.length - newFiles.length); // Select first newly added
...
function renderThumbnailTray() {
  if (currentFiles.length <= 1) {
    thumbnailTray.style.display = "none";
    return;
  }
  thumbnailTray.style.display = "flex";
  ...
  currentFiles.forEach((file, index) => {
    ...
    img.style.border = index === currentFileIndex ? "2px solid var(--primary)" : "2px solid transparent";
    img.onclick = () => selectThumbnail(index);
    thumbnailTray.appendChild(img);
  });
}
...
function selectThumbnail(index) {
  ...
  if (currentFiles.length > 1) {
     Array.from(thumbnailTray.children).forEach((child, i) => {
        child.style.border = i === currentFileIndex ? "2px solid var(--primary)" : "2px solid transparent";
     });
  }
}
...
clearBtn.addEventListener("click", () => {
  ...
  if (currentFiles.length === 0) {
      ...
      renderThumbnailTray();
      selectThumbnail(Math.max(0, currentFileIndex - 1));
  }
});
